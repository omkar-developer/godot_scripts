...
var avoidance_multiplier := 300.0
var sperad = PI/3.0
...
func calculate() -> Vector2:
    ...
    if forward_collision:
        ...
        # Use collision normal directly
        avoidance_force += forward_collision.normal * force_strength * 2.0
    
    # Left ray
    var left_dir = velocity_dir.rotated(-sperad)  # 45 degrees left
    ...
    if left_collision:
        ...
        # Use collision normal instead of fixed direction
        avoidance_force += left_collision.normal * force_strength
    
    # Right ray
    var right_dir = velocity_dir.rotated(sperad)  # 45 degrees right
    ...
    if right_collision:
        ...
        var force_strength = (side_ray_length - distance) / side_ray_length
        # Use collision normal instead of fixed direction
        avoidance_force += right_collision.normal * force_strength
    
    # Apply multiplier and limit force
